...
def predict(test_data, results):
    ...
    compared_resuts = predict(test_data, results)
    """
    model_params = DataFrame(results[0].params)
    formula = results[1]

    # Create regression friendly test DataFrame
    yt, xt = dmatrices(formula, data=test_data, return_type='dataframe')
    xt, model_params = get_dataframes_intersections(xt, xt.columns,model_params.transpose(), model_params.index)
